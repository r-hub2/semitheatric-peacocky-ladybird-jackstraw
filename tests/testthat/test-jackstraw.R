...
test_that( "jackstraw_lfa works with BEDMatrix", {
    # NOTE: Normally I wrap these around `expect_silent`, but that's resulting in an unexpected WinBuilder-only error, and that isn't informative enough to tell me what is going on so I can fix it properly.  I ran the tests myself on a Windows computer and didn't reproduce any issues!  This is super strange, but I'm giving up and just running these without expectation that they be quiet
